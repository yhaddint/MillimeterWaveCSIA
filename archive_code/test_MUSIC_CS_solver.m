...
% System Parameters
%-------------------------------------
path_num = 2; % Num of rays in a cluster
...
AODspread = 0;
SNR_num = 1;
SNR_data_range = linspace(10,10,SNR_num);
SNR_range = SNR_data_range + 0; % There is 1000x diff. in IA and data AWGN BW
...
%     g_ray0 = randn(path_num,1) + 1j*randn(path_num,1);
    g_ray0 = exp(1j*2*pi*randn(path_num,1));
    ...
    for pp=1:path_num
        [~,row_true] = min(abs(cand_angle_r - phi(MCindex, pp)));
        [~,col_true] = min(abs(cand_angle_t - theta(MCindex, pp)));
        index_true(pp) = (col_true-1)*cand_num_r + row_true;
    end
    ...
    for M_index=1:length(M_range)
        ...
        %%
        for ll=1:path_num
            chan_eff(:,ll) = g_ray(ll)*(W'*arx(:,ll)) .* conj(F'*atx(:,ll));
        end
        ...
        for ss = 1:SNR_num
            ...
            % Covariance matrix constructuion
            sig_Cov = (sig_rx*sig_rx') + eye(M)*sigman2;
%             sig_Cov = (chan_eff(:,1)*chan_eff(:,1)')...
%                     + (chan_eff(:,2)*chan_eff(:,2)')...
%                     + (chan_eff(:,1)*chan_eff(:,2)')...
%                     + (chan_eff(:,2)*chan_eff(:,1)');%...
%                     + eye(M)*sigman2;
            ...
            % MUSIC solver
%             NS_index = (path_num+1):M; % Index for nonspace
%             [U,Sigma,V] = svd(sig_Cov); % SVD on Covariance
%             NS_mtx = U(:,NS_index); % Signal nonspace
%             for dd=1:dict_num
%                 score_final(dd) = (Measure_mat_new(:,dd)'*Measure_mat_new(:,dd))/...
%                     norm(NS_mtx'*Measure_mat_new(:,dd))^2;
%             end
            
            % Plot "spectrum" in dictionary search
            debug_flag = 1;
            if debug_flag
                figure
                plot(10*log10(abs(score_final)));hold on;
                plot(index_true,10*log10(abs(score_final(index_true))),'o','linewidth',2,'markersize',10);
                grid on
                legend('All Scores','True AoA/AoD pair')
            end
