...
M_burst = [8, 4]; % Number of bursts in IA; For directional use [M_Tx_BF,M_Rx_BF] for beams in BS and UE, otherwise M equals to M_Tx_BF*M_Rx_BF
...
figure
subplot(211)
plot((1:length(Rx_sig_H1))/burst_N,abs(Rx_sig_H1));hold on
grid on
...
subplot(212)
plot((1:length(corr_out_H1_STO(ZC_N+CP:end)))/burst_N,abs(corr_out_H1_STO(ZC_N+CP:end)));hold on
grid on
xlabel('SS Burst Index')
